...
export function InteractiveText({text, findings, entities, activeId, onSelect, enabled}: Props) {
  ...
  const rangesWithKind = [...ranges.map(finding => ({...finding, kind:"finding" as const})), ...entities.filter(entity => entity.endIndex > entity.startIndex).map(entity => ({...entity, kind:"entity" as const}))];
