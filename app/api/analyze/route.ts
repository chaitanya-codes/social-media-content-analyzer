...
export async function POST(request: Request) {
  try {
    ...
    else if (["image/png","image/jpeg","image/webp"].includes(file.type)) { sourceType="image"; extractionMethod="ocr"; const worker = await createWorker("eng"); await worker.setParameters({tessedit_pageseg_mode:PSM.AUTO, preserve_interword_spaces:"0"}); const result = await worker.recognize(buffer); text = normalizeText(result.data.text || ""); extractionQuality = result.data.confidence >= 80 ? "high" : result.data.confidence >= 55 ? "medium" : "low"; await worker.terminate(); }
